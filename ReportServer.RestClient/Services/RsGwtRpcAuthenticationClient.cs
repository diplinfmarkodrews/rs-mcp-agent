...
public class RsRestAuthenticationClient : RsRestClientBase
{
    ...
    // Constructor
    public RsRestAuthenticationClient(HttpClient httpClient, CookieContainerProvider cookieProvider)
    ...
    public async Task<GwtRpcResponse<string>> LogoutAsync()
    {
        try
        {
            var payload = BuildGwtRpcPayload(
            ...
            {
                _cookieContainer.GetCookies(_httpClient.BaseAddress).Cast<Cookie>().ToList()
                    .ForEach(cookie => cookie.Expired = true);
            }

            return GwtRpcResponse<string>.Successful(response, "Logout successful");
        }
        catch (Exception ex)
        {
