...
public class McpServerHostedService : BackgroundService
{
    ...
    public McpServerHostedService(ILogger<McpServerHostedService> logger, IReportServer reportServer)
    {
        _logger = logger;
        ...
        try
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    _logger.LogDebug("Health check: {Status}", health ? "Healthy" : "Unhealthy");
                }
                catch (Exception ex)
                {
