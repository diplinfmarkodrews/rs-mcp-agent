...
public class BrowserStreamingService : IBrowserStreamingService, IDisposable
{
    ...
    public async Task<string> CreateStreamingSessionAsync(string? initialUrl = null)
    {
        ...
        try
        {
            ...
            var context = browserInstance.BrowserContext;
            var page = browserInstance.Page;
            ...
            page.Load += async (sender, e) => await OnPageLoad(sessionId);
            page.DOMContentLoaded += async (sender, e) => await OnDOMContentLoaded(sessionId);
            page.Console += async (sender, e) => await OnConsoleMessage(sessionId, e);
    ...
    public async Task CloseStreamingSessionAsync(string sessionId)
    {
        if (_sessions.TryRemove(sessionId, out var session))
        {
            try
            {
                ...
                session.Page.Load -= async (sender, e) => await OnPageLoad(sessionId);
                session.Page.DOMContentLoaded -= async (sender, e) => await OnDOMContentLoaded(sessionId);
                session.Page.Console -= async (sender, e) => await OnConsoleMessage(sessionId, e);
        ...
        try
        {
            ...
            switch (interaction.Type.ToLowerInvariant())
            {
                case "click":
                    ...
                    {
                        await session.Page.Mouse.ClickAsync((float)interaction.X.Value, (float)interaction.Y.Value);
                    }
                    else if (!string.IsNullOrEmpty(interaction.Selector))
                    {
                        await session.Page.ClickAsync(interaction.Selector);
                    }
                    break;
                ...
                case "type":
                    if (!string.IsNullOrEmpty(interaction.Text))
                    {
                        ...
                        {
                            await session.Page.FillAsync(interaction.Selector, interaction.Text);
                        }
                        else
                        {
                            await session.Page.Keyboard.TypeAsync(interaction.Text);
                        }
                    }
                    break;
                ...
                case "keypress":
                    if (!string.IsNullOrEmpty(interaction.Key))
                    {
                        ...
                        if (interaction.CtrlKey)
                            await session.Page.Keyboard.DownAsync("Control");
                        if (interaction.ShiftKey)
                            await session.Page.Keyboard.DownAsync("Shift");
                        if (interaction.AltKey)
                            await session.Page.Keyboard.DownAsync("Alt");

                        // Press the main key
                        await session.Page.Keyboard.PressAsync(interaction.Key);

                        // Release modifier keys
                        if (interaction.AltKey)
                            await session.Page.Keyboard.UpAsync("Alt");
                        if (interaction.ShiftKey)
                            await session.Page.Keyboard.UpAsync("Shift");
                        if (interaction.CtrlKey)
                            await session.Page.Keyboard.UpAsync("Control");
                    }
                    break;
                ...
                case "scroll":
                    if (interaction.X.HasValue && interaction.Y.HasValue)
                    {
                        await session.Page.Mouse.WheelAsync((float)interaction.X.Value, (float)interaction.Y.Value);
                    }
                    break;
            }
    ...
    private async Task CaptureAndBroadcastFrame(string sessionId, BrowserStreamingSession session)
    {
        try
        {
            var screenshot = await session.Page.ScreenshotAsync(new PageScreenshotOptions
            {
                Type = ScreenshotType.Jpeg,
                Quality = 75, // Reduce quality for better streaming performance
    ...
    private async Task OnDOMContentLoaded(string sessionId)
    {
        if (_sessions.TryGetValue(sessionId, out var session))
        {
            await BroadcastPageUpdate(sessionId, session);
    ...
    private async Task OnConsoleMessage(string sessionId, IConsoleMessage message)
    {
        _logger.LogDebug("Console message from session {SessionId}: {Type} - {Text}", 
            sessionId, message.Type, message.Text);
...
// Supporting classes
public class BrowserStreamingSession : IDisposable
{
    ...
    public void Dispose()
    {
        // Context and Page disposal is handled by the browser instance provider
        // We only clean up session-specific resources here
    }
}
